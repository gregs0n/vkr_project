...
def CreateBoundFunc(func_num: int,
                    limits: list,
    ...
    u_test = [lambda x, y: 0.5*coef*(x + y) + d,
    ...
    du = [[lambda x, y: 0.5*coef, lambda x, y: 0.5*coef],
      [lambda x, y: 0.5*coef*2*x, lambda x, y: 0.5*coef],
      [lambda x, y: 0.5*coef*2*x, lambda x, y: 0.5*coef*2*y],
      [lambda x, y: coef*4*x**3*y**3, lambda x, y: coef*3*x**4*y**2],
    ...
    d2u = [[lambda x, y: 0*x*y, lambda x, y: 0*x*y],
       [lambda x, y: 0.5*coef*2, lambda x, y: 0*x*y],
       [lambda x, y: 0.5*coef*2*(1.0 + x - x), lambda x, y: 0.5*coef*2*(1.0 + x - x)],
    ...
    def g(x, y):
        if (x == 0):
            if (y == 0):
                return 0.5*(2*stef_bolc*fabs(u(0, 0))*fpower(u(0, 0), 3) \
                    - tcc*normed*dux(0, 0) - tcc*normed*duy(0, 0))
            elif (y == limits[1]):
                return 0.5*(2*stef_bolc*fabs(u(0, limits[1]))*fpower(u(0, limits[1]), 3) \
                    - tcc*normed*dux(0, limits[1]) + tcc*normed*duy(0, limits[1]))
            else:
                return stef_bolc*fabs(u(x, y))*fpower(u(x,y), 3) - tcc*normed * dux(x, y)
        elif (x == limits[0]):
            if (y == 0):
                return 0.5*(2*stef_bolc*fabs(u(limits[0], 0))*fpower(u(limits[0], 0), 3) \
                    + tcc*normed*dux(limits[0], 0) - tcc*normed*duy(limits[0], 0))
            elif (y == limits[1]):
                return 0.5*(2*stef_bolc*fabs(u(limits[0], limits[1]))*fpower(u(limits[0], limits[1]), 3) \
                    + tcc*normed*dux(limits[0], limits[1]) + tcc*normed*duy(limits[0], limits[1]))
            else:
                return stef_bolc*fabs(u(x, y))*fpower(u(x,y), 3) + tcc*normed * dux(x, y)
        elif (y == 0):
...
def SpecialBoundFunc(func_num: int,
    ...
    coef = tmax - tmin
    d = tmin
    gs = [lambda x, y: __Hs(d + coef*sin(pi*x/0.3)) if y == 0.0 and 0.0 <= x <= 0.3 else __Hs(tmin),
